...
for i in $files
do
#  ../udp/udpipe-1.2.0-bin/bin-linux64/udpipe --tokenize --tag --parse $modelfile $datadir$i --output=conllu --outfile=$outputdir$i".parsed.txt"
  ../udp/udpipe.exe --tokenize --tag --parse $modelfile $datadir$i --output=conllu --outfile=$outputdir$i".parsed.txt"
